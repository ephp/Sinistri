...
namespace Ephp\Bundle\WsInvokerBundle\PhpExcel;
...
class SpreadsheetExcelReader {
    ...
    function __construct($file = '', $store_extended_info = true, $outputEncoding = '') {
        $this->_defaultFormat = Utility::$SPREADSHEET_EXCEL_READER_DEF_NUM_FORMAT;
    ...
    function _parse() {
        ...
        $code = Utility::v($data, $pos);
        $length = Utility::v($data, $pos + 2);
        $version = Utility::v($data, $pos + 4);
        $substreamType = Utility::v($data, $pos + 6);

        $this->version = $version;

        if (($version != Utility::$SPREADSHEET_EXCEL_READER_BIFF8) &&
                ($version != Utility::$SPREADSHEET_EXCEL_READER_BIFF7)) {
            return false;
        }

        if ($substreamType != Utility::$SPREADSHEET_EXCEL_READER_WORKBOOKGLOBALS) {
            return false;
        }
        ...
        while ($code != Utility::$SPREADSHEET_EXCEL_READER_TYPE_EOF) {
            switch ($code) {
                case Utility::$SPREADSHEET_EXCEL_READER_TYPE_SST:
                    $spos = $pos + 4;
                    ...
                    for ($i = 0; $i < $uniqueStrings; $i++) {
                        ...
                        if ($spos == $limitpos) {
                            $opcode = Utility::v($data, $spos);
                            $conlength = Utility::v($data, $spos + 2);
                            if ($opcode != 0x3c) {
                                return -1;
                            }
                            $spos += 4;
                        ...
                        if ($richString) {
                            // Read in the crun
                            $formattingRuns = Utility::v($data, $spos);
                            $spos += 2;
                        }
                    ...
                    break;
                case Utility::$SPREADSHEET_EXCEL_READER_TYPE_FILEPASS:
                    return false;
                    break;
                case Utility::$SPREADSHEET_EXCEL_READER_TYPE_NAME:
                    break;
                case Utility::$SPREADSHEET_EXCEL_READER_TYPE_FORMAT:
                    $indexCode = Utility::v($data, $pos + 4);
                    if ($version == Utility::$SPREADSHEET_EXCEL_READER_BIFF8) {
                        $numchars = Utility::v($data, $pos + 6);
                        if (ord($data[$pos + 8]) == 0) {
                    ...
                    break;
                case Utility::$SPREADSHEET_EXCEL_READER_TYPE_FONT:
                    $height = Utility::v($data, $pos + 4);
                    $option = Utility::v($data, $pos + 6);
                    $color = Utility::v($data, $pos + 8);
                    $weight = Utility::v($data, $pos + 10);
                ...
                case Utility::$SPREADSHEET_EXCEL_READER_TYPE_PALETTE:
                ...
                case Utility::$SPREADSHEET_EXCEL_READER_TYPE_XF:
...
//						$bgcolor = ($bgi & 0x3f80) >> 7;
                    ...
                    $this->xfRecords[] = $xf;
                    break;
                case Utility::$SPREADSHEET_EXCEL_READER_TYPE_NINETEENFOUR:
                    $this->nineteenFour = (ord($data[$pos + 4]) == 1);
                    break;
                case Utility::$SPREADSHEET_EXCEL_READER_TYPE_BOUNDSHEET:
                    ...
                    } elseif ($version == Utility::$SPREADSHEET_EXCEL_READER_BIFF7) {
    ...
    function _parsesheet($spos) {
        ...
        if (($version != Utility::$SPREADSHEET_EXCEL_READER_BIFF8) && ($version != Utility::$SPREADSHEET_EXCEL_READER_BIFF7)) {
            return -1;
        }

        if ($substreamType != Utility::$SPREADSHEET_EXCEL_READER_WORKSHEET) {
            return -2;
        }
        $spos += $length + 4;
        while ($cont) {
            $lowcode = ord($data[$spos]);
            if ($lowcode == Utility::$SPREADSHEET_EXCEL_READER_TYPE_EOF)
                break;
            ...
            unset($this->rectype);
            switch ($code) {
                case Utility::$SPREADSHEET_EXCEL_READER_TYPE_DIMENSION:
                    if (!isset($this->numRows)) {
                        if (($length == 10) || ($version == Utility::$SPREADSHEET_EXCEL_READER_BIFF7)) {
                    ...
                    break;
                case Utility::$SPREADSHEET_EXCEL_READER_TYPE_MERGEDCELLS:
                    ...
                    break;
                case Utility::$SPREADSHEET_EXCEL_READER_TYPE_RK:
                case Utility::$SPREADSHEET_EXCEL_READER_TYPE_RK2:
                    ...
                    break;
                case Utility::$SPREADSHEET_EXCEL_READER_TYPE_LABELSST:
                    $row = ord($data[$spos]) | ord($data[$spos + 1]) << 8;
                    $column = ord($data[$spos + 2]) | ord($data[$spos + 3]) << 8;
                    ...
                    break;
                case Utility::$SPREADSHEET_EXCEL_READER_TYPE_MULRK:
                    $row = ord($data[$spos]) | ord($data[$spos + 1]) << 8;
                    ...
                    break;
                case Utility::$SPREADSHEET_EXCEL_READER_TYPE_NUMBER:
                    $row = ord($data[$spos]) | ord($data[$spos + 1]) << 8;
                    $column = ord($data[$spos + 2]) | ord($data[$spos + 3]) << 8;
                ...
                case Utility::$SPREADSHEET_EXCEL_READER_TYPE_FORMULA:
                case Utility::$SPREADSHEET_EXCEL_READER_TYPE_FORMULA2:
                    $row = ord($data[$spos]) | ord($data[$spos + 1]) << 8;
                    $column = ord($data[$spos + 2]) | ord($data[$spos + 3]) << 8;
                    ...
                    break;
                case Utility::$SPREADSHEET_EXCEL_READER_TYPE_BOOLERR:
                    $row = ord($data[$spos]) | ord($data[$spos + 1]) << 8;
                    $column = ord($data[$spos + 2]) | ord($data[$spos + 3]) << 8;
                    $string = ord($data[$spos + 6]);
                    $this->addcell($row, $column, $string);
                    break;
                case Utility::$SPREADSHEET_EXCEL_READER_TYPE_STRING:
                    // http://code.google.com/p/php-excel-reader/issues/detail?id=4
                    if ($version == Utility::$SPREADSHEET_EXCEL_READER_BIFF8) {
                    ...
                    break;
                case Utility::$SPREADSHEET_EXCEL_READER_TYPE_ROW:
                    $row = ord($data[$spos]) | ord($data[$spos + 1]) << 8;
                    ...
                    break;
                case Utility::$SPREADSHEET_EXCEL_READER_TYPE_DBCELL:
                    break;
                case Utility::$SPREADSHEET_EXCEL_READER_TYPE_MULBLANK:
                    $row = ord($data[$spos]) | ord($data[$spos + 1]) << 8;
                    $column = ord($data[$spos + 2]) | ord($data[$spos + 3]) << 8;
                    $cols = ($length / 2) - 3;
                    for ($c = 0; $c < $cols; $c++) {
                    ...
                    break;
                case Utility::$SPREADSHEET_EXCEL_READER_TYPE_LABEL:
                    $row = ord($data[$spos]) | ord($data[$spos + 1]) << 8;
                    $column = ord($data[$spos + 2]) | ord($data[$spos + 3]) << 8;
                    $this->addcell($row, $column, substr($data, $spos + 8, ord($data[$spos + 6]) | ord($data[$spos + 7]) << 8));
                    break;
                case Utility::$SPREADSHEET_EXCEL_READER_TYPE_EOF:
                    $cont = false;
                    break;
                case Utility::$SPREADSHEET_EXCEL_READER_TYPE_HYPER:
                    ...
                    break;
                case Utility::$SPREADSHEET_EXCEL_READER_TYPE_DEFCOLWIDTH:
                    $this->defaultColWidth = ord($data[$spos + 4]) | ord($data[$spos + 5]) << 8;
                    break;
                case Utility::$SPREADSHEET_EXCEL_READER_TYPE_STANDARDWIDTH:
                    $this->standardColWidth = ord($data[$spos + 4]) | ord($data[$spos + 5]) << 8;
                    break;
                case Utility::$SPREADSHEET_EXCEL_READER_TYPE_COLINFO:
    ...
    function _getCellDetails($spos, $numValue, $column) {
        ...
        if ($type == 'date') {
            ...
            $utcDays = floor($numValue - ($this->nineteenFour ? Utility::$SPREADSHEET_EXCEL_READER_UTCOFFSETDAYS1904 : Utility::$SPREADSHEET_EXCEL_READER_UTCOFFSETDAYS));
            $utcValue = ($utcDays) * Utility::$SPREADSHEET_EXCEL_READER_MSINADAY;
            $dateinfo = Utility::gmgetdate($utcValue);

            $raw = $numValue;
            $fractionalDay = $numValue - floor($numValue) + .0000001; // The .0000001 is to fix for php/excel fractional diffs

            $totalseconds = floor(Utility::$SPREADSHEET_EXCEL_READER_MSINADAY * $fractionalDay);
