...
<div class="btn success fileinput-button left" id="<?php echo $id ?>_bt_adde">
    <button class="no-border no-margin left"><span class="attachment no-margin left"></span><small class="left">Allega uno o più file</small></button>
...
<script type="text/javascript">  
    ...
    .bind('fileuploaddone', function (e, data) {
        ...
        if(response.form_id == drag_drop_form_id) {
            arr = eval($('#<?php echo $field ?>').val());
            arr.add(response.name);
            $('#<?php echo $field ?>').val(serialize(arr));
...
//        $('#<?php echo $id ?>_bt_add').show();
    ...
    function addDocumento(data) {
        msg = "<li class='doc_row_small' id='allegato_"+n_file+"'><a href='"+data.url+"' target='_blank'>"+data.name+"</a> <a class='delete-item left margin-right-10' href='javascript:remove(\""+data.delete_url+"\", \""+data.name+"\", "+n_file+")'>[X]</a></li>"
        $('#docs').prepend(msg);
    }
