...
<div class="btn success fileinput-button left relative" id="<?php echo $id ?>_bt_adde">
    <button class="button-orange smaller no-margin left" id="load_documenti" type="button">Seleziona i documenti che vuoi caricare</button>
...
//        $('#<?php echo $id ?>_bt_add').show();
    ...
    function addDocumento(data) {
        $.post(url_add_documento, data, function(msg){
            ...
            $('#docs').prepend(msg);
            doc = formStaticFind(msg);
            doc = 'doc_'+doc.toString().to(doc.length - 8).from(10);
            enable_new_edit('documenti_large', doc);
            attiva_documento(doc.from(4));
            todoFatto('documento');
            resetAddButton();
            });
