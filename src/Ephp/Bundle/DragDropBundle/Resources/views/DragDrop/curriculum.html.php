...
<div class="btn success fileinput-button left relative" id="<?php echo $id ?>_bt_adde">
    <button class="button-orange smaller no-margin left" type="button">Carica il tuo curriculum</button>
    <div class="progressbar fileupload-progressbar absolute " style="width: 140px; top: 2px; right: -150px;"><div style="width:0%;"></div></div>
                ...
                $.each(files, function (index, file) {
                    if(that._hasError(file)) {
                        switch(that._hasError(file)) {
                            case 'minFileSize':
                                fancyAlert('Il documento ' +file.name+' è troppo piccolo (min: 1Kb)');
                                break;
                            case 'maxFileSize':
                                fancyAlert('Il documento ' +file.name+' è troppo grande (max: 7Mb)');
                                break;
                            case 'acceptFileTypes':
                                fancyAlert('Il documento ' +file.name+' non è del tipo accettato<br>(File accettati: Acrobat Pdf, Office Word, OpenOffice Writer)');
                                break;
                            default:
...
//        $('#<?php echo $id ?>_bt_add').show();
    ...
    function addDocumento(data) {  
        data["sezione"]="cv";
         ...
         $.post(url_add_documento, data, function(msg){
        ...
        if($('#<?php echo $field ?>_delete').val() != '') {
            removeFile($('#<?php echo $field ?>_delete').val());
        }
        $('#<?php echo $field ?>').val(data.url);
        $('#<?php echo $field ?>_delete').val(data.delete_url);
        
        $('#docs').html('<small>Il tuo Curriculum è stato caricato</small>');
    }
